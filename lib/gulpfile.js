module.exports = (program) => {
  ...
  // check registry block
  {
    ...
    ['npm'].map(item => {
      ...
      if(['https://rcnpm.zhuanspirit.com/','http://rcnpm.zhuaninc.com/'].includes(registry.replace(/\s/g,''))) checkRegistryPass = true;
    })
    if(!checkRegistryPass) throw `请设置：npm config set registry https://rcnpm.zhuanspirit.com`;
  }
  ...
  function compile(modules) {
    ...
    const assets = gulp.src(['src/**/*.@(png|svg|css)']).pipe(gulp.dest(modules === false ? esDir : libDir));
    const source = [
      'src/**/*.js',
    ];
  ...
  gulp.task('build-doc', function (cb) {
    ...
    try {
      config = require(path.join(process.cwd(), 'jsdoc.conf.json'));
    } catch (error) {
    ...
    gulp.src([`${process.cwd()}/README.md`])
      ...
      .on('end', () => {
        ...
        gulp.src([`${process.cwd()}/README.md`, `${process.cwd()}/src/**/*.js`, `${process.cwd()}/src/**/*.ts`], {read: false})
          .pipe(jsdoc(merge(configOri, config), function() {
  ...
  gulp.task('doc-upload', ['build-doc'], function () {
    ...
    const pipe = gulp.src(`${process.cwd()}/${ program.docsDirName || 'docs' }/**/*`)
      .pipe(conn.dest(`/${program.groupName || 'zz-common'}/${packageJson.name}/`))
      .on('end', () => {
        const open = require('open')
        open(`https://fe.zhuanspirit.com/${program.groupName || 'zz-common'}/${packageJson.name}`)
      });
    return pipe
  });
  ...
  gulp.task('dev', function (done) {
    ...
    const execDev = () => {
      ...
      const server = new WebpackDevServer(compiler,{
        ...
        publicPath: `/zz-common/${packageJson.name}/`,
      ...
      server.listen(port);
      open(`http://127.0.0.1:${port}/zz-common/${packageJson.name}/`);

      new Promise(()=>{
        compiler.hooks.done.tap('dev', stats => {
          console.log(`open at: http://127.0.0.1:${port}/zz-common/${packageJson.name}/`)
        })
      })
    };
