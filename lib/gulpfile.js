module.exports = program => {
    ...
    gulp
      ...
      .on("end", () => {
        ...
        gulp
          ...
          .pipe(
            jsdoc(merge(configOri, config), function() {
  ...
  gulp.task("doc-upload", ["build-doc"], function() {
    ...
    const pipe = gulp
      ...
      .pipe(
        conn.dest(`/${program.groupName || "zz-common"}/${packageJson.name}/`)
      )
      .on("end", () => {
        const open = require("open");
        open(
          `https://fe.zhuanspirit.com/${program.groupName || "zz-common"}/${
            packageJson.name
          }`
        );
      });
    ...
    const execDev = () => {
      ...
      new Promise(() => {
        compiler.hooks.done.tap("dev", stats => {
          console.log(
            `open at: http://127.0.0.1:${port}/zz-common/${packageJson.name}/`
          );
        });
      });
    };
