module.exports = program => {
  ...
  function dist(done) {
    ...
    let webpackConfig = require('./config/webpack.prod.config.js')(program.single, program.analyz)
    if (fs.existsSync(path.join(cwd, 'webpack.config.js'))) {
      webpackConfig.map(config => {
        return webpackMerge(config, require(path.join(cwd, 'webpack.config.js')))
      })
    }
  ...
  gulp.task('commitlint', done => {
    ...
    runCmd('commitlint', ['--config', configFile, '-E', 'HUSKY_GIT_PARAMS'], code => {
      code ? process.exit(code) : done()
    })
  })
