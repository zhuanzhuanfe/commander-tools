module.exports = program => {
  ...
  function dist(done) {
    ...
    let webpackConfig = getWebpackConfig(program.single, program.analyz)
  ...
  function compile() {
    ...
    const filesStream = babelify(result, useESModules)
  ...
  gulp.task('lint', done => {
    ...
    if (staged) {
      if (fix) process.env.ESLINT_FIX = 'true'
      const configFile = path.join(__dirname, 'lint-staged.config.js')
      runCmd('lint-staged', ['--config', configFile], () => {
        done()
      })
    } else {
      const configFile = path.join(cwd, '.eslintrc.js')
      const args = ['--config', configFile, 'src/**']
  ...
  gulp.task('commitlint', done => {
    console.log('running commitlint')

    const configFile = path.join(__dirname, 'commitlint.config.js')
    runCmd('commitlint', ['--config', configFile, '-E', 'HUSKY_GIT_PARAMS'], () => {
      done()
    })
  })
  ...
  gulp.task('dev', () => {
    ...
    const execDev = () => {
      ...
      if (fs.existsSync(path.join(cwd, 'webpack.config.js'))) {
        webpackConfig = webpackMerge(webpackConfig, require(path.join(cwd, 'webpack.config.js')))
      }
      ...
      const server = new WebpackDevServer(compiler, {
        ...
        // open: true,
        // openPage: `${pkgname}/`,
        publicPath: `/${businessLine}/${pkgname}/`
      });
      ...
      server.listen(port)
      open(`https://127.0.0.1:${port}/${businessLine}/${pkgname}/`)

      return new Promise(() => {
        compiler.hooks.done.tap('dev', () => {
          console.log(
            `open at: https://127.0.0.1:${port}/${businessLine}/${pkgname}/`
          )
        })
      })
    }
  ...
  gulp.task('compile-lib', () => {
    console.log('running compile-lib')
  ...
  gulp.task('compile-es', () => {
    console.log('running compile-es')
  ...
  } else if (existsSync(docConfigDir)) {
  ...
  gulp.task('doc-upload', gulp.series('build-doc', () => {
    console.log('running doc-upload')
