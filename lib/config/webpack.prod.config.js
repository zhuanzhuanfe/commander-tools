...
files.forEach((val) => {
  ...
  const noIndexName = /\.((js)|(ts))$/.test(val) && (val !== 'index.js' || val !== 'index.ts')
  if(stats.isFile() && noIndexName) {
    entryObj.push({
      name: val.replace('.js', '').replace('.ts', ''),
      file: [filePath],
...
module.exports = function(single, analyz, keepconsole = false) {
